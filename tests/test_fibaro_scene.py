...
def test_fibaro_scene_start_v5() -> None:
    """Test get request"""
    with requests_mock.Mocker() as mock:
        assert isinstance(mock, requests_mock.Mocker)

        info_payload_v5 = info_payload.copy()
        info_payload_v5["serialNumber"] = "HC3-111111"

        mock.register_uri("GET", f"{TEST_BASE_URL}scenes", json=scene_payload)
        mock.register_uri("POST", f"{TEST_BASE_URL}scenes/2/execute")
        mock.register_uri("GET", f"{TEST_BASE_URL}loginStatus", json=login_payload)
        mock.register_uri("GET", f"{TEST_BASE_URL}settings/info", json=info_payload_v5)
...
def test_fibaro_scene_stop_v5() -> None:
    """Test get request"""
    with requests_mock.Mocker() as mock:
        assert isinstance(mock, requests_mock.Mocker)
        ...
        mock.register_uri("GET", f"{TEST_BASE_URL}scenes", json=scene_payload)
        mock.register_uri("POST", f"{TEST_BASE_URL}scenes/2/kill")
        mock.register_uri("GET", f"{TEST_BASE_URL}loginStatus", json=login_payload)
        mock.register_uri("GET", f"{TEST_BASE_URL}settings/info", json=info_payload_v5)
